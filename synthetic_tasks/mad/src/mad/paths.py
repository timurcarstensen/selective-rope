...
def parse_path(path: str, element_sep: str = "_", key_value_sep: str = "-") -> dict:
    """
    ...
    values = [
        f"{key_value_sep}".join(e.split(key_value_sep)[1:]) for e in path_elements
    ]
    parsed_path = {}
    for k, v in zip(keys, values):
        ...
        if k == "task":
            try:
                task_shorthands = np.array(
                    [e["shorthand"] for e in task_registry.values()]
                )
...
def make_log_path(
    base_path: str, mad_config, model_id: str, add_timestamp: bool = False, **kwargs
) -> str:
    """
