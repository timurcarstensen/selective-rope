...
class PLModelWrap(pl.LightningModule):
    ...
    def instantiate_metrics(self, metrics: list) -> None:
        ...
        for m in metrics:
            if m == "acc":
                ...
                raise ValueError(
                    f"invalid metric: {m}, must be one of 'acc', 'ppl' or a torchmetrics metric instance"
                )
    ...
    def phase_step(
        self, batch: tuple, batch_idx: int, phase: str = "train"
        ...
        self.log_dict(
            metrics, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True
        )
    ...
    def test_step(self, batch: tuple, batch_idx: int) -> dict[str, torch.Tensor | Any]:
        return self.phase_step(batch, batch_idx, phase="test")
