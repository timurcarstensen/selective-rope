...
class AutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert global_pool in {"avg", "cls", "last"}, (
            'global_pool must be one of "avg", "cls" or "last"'
        )
        assert len(layer_cfgs) == len(layers), (
            "number of layer configs must be equal to number of layers"
        )
        assert all(cfg["dim"] == dim for cfg in layer_cfgs), (
            "all layer configs must have the same dimensionality"
        )
        assert all(cfg["max_length"] == max_length for cfg in layer_cfgs), (
            "all layer configs must have the same max_length"
        )

        self.vocab_size = vocab_size
        self.token_embeds = nn.Embedding(vocab_size, dim)
        enc_posembs = (
            position_embeds(max_length, dim) if position_embeds is not None else None
        )
        self.enc_position_embeds = (
            enc_posembs.weight if isinstance(enc_posembs, nn.Embedding) else enc_posembs
        )
        assert self.enc_position_embeds is None or self.enc_position_embeds.shape == (
            max_length,
            dim,
        ...
        for layer, layer_cfg in zip(layers, layer_cfgs):
            layer_cfg["max_length"] = (
                max_length + 1 if global_pool == "cls" else max_length
            )
            self.encoder.append(
                nn.Sequential(norm(layer_cfg["dim"]), layer(**layer_cfg))
            )
    ...
    def embed(
        self, inputs_ids: torch.Tensor, position_ids: torch.Tensor = None
    ) -> torch.Tensor:
    ...
    def decode(
        self, encoding: torch.Tensor, position_ids: torch.Tensor = None
    ) -> torch.Tensor:

        batch_size, dim = encoding.shape
        x = (
            encoding.unsqueeze(1)
            + self.dec_position_embeds.to(encoding.device)[position_ids]
        )
