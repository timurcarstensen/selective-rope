...
class LanguageModel(nn.Module):
    ...
    Args:
        ...
        position_embeds (Callable, optional): Positional embeddings.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert len(layer_cfgs) == len(layers), (
            "number of layer configs must be equal number of layers"
        )
        assert all(cfg["dim"] == dim for cfg in layer_cfgs), (
            "all layer configs must specify the same dimensionality"
        )
        ...
        self.position_embeds = (
            position_embeds.weight
            if isinstance(position_embeds, nn.Embedding)
            else position_embeds
        )
        assert self.position_embeds is None or self.position_embeds.shape == (
            max_length,
            dim,
