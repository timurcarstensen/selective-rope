...
from mad.model.layers.ops.gla.chunk_util import (
    bwd_decay_global_cumsum,
    fwd_decay_cumsum,
    prepare_qg_kg,
)
from mad.model.layers.ops.gla.utils import contiguous
...
def chunk_gla_bwd_kernel(
    ...
    mask = (i_k * BK + tl.arange(0, BK)[:, None] < DK) & (
        i_v * BV + tl.arange(0, BV)[None, :] < DV
    )

    p_dh = (
        dh
        + i_bh * s_hh
        + (TDK - DK + i_k * BK + tl.arange(0, BK)[:, None]) * DV
        + i_v * BV
        + tl.arange(0, BV)[None, :]
    )
...
def fwd_inner_chunk(
    ...
    if i_i > i_j:
        ...
        o_A = (
            (i_bh + i_k * n_bh) * s_A_h
            + i_t * s_A_c
            + (i_i * BC + tl.arange(0, BC)) * BT
        )
        # first position right.
        # [BK,]
        b_gn = tl.load(
            g + i_bh * s_qk_h + (i_t * BT + i_i * BC) * DK + tl.arange(0, BK) + i_k * BK
        )
        ...
        tl.store(
            A + o_A[:, None] + tl.arange(0, BC) + i_j * BC, s.to(A.dtype.element_ty)
        )
    # TODO: better way to utilize idle kernel instances
        ...
        o_k = i_bh * s_qk_h + (i_t * BT + i_i * BC) * DK + tl.arange(0, BK) + i_k * BK
        o_A = (
            (i_bh + i_k * n_bh) * s_A_h
            + i_t * s_A_c
            + (i_i * BC + tl.arange(0, BC)) * BT
        ...
        for i in range(0, BC):
            ...
            # [BC,]
            s = tl.sum(
                (b_q * b_k_i[None, :] * tl.math.exp2(b_gq - b_gk_i[None, :])), axis=1
            )
...
def bwd_inner_chunk(
    ...
    for i in range(0, BC):
        ...
        dA_ij = tl.load(
            ...
            + i_bh * s_A_h
            + i_t * s_A_c
            + (i_c * BC + tl.arange(0, BC)) * BT
            + i
            + i_c * BC
        )
        dA_ij = tl.where(o_i >= i, dA_ij, 0)
        k_j = tl.load(
            k + i_bh * s_qk_h + (i_t * BT + i_c * BC + i) * DK + tl.arange(0, DK)
        )
        gk_j = tl.load(
            g + i_bh * s_qk_h + (i_t * BT + i_c * BC + i) * DK + tl.arange(0, DK)
        )
        dq_i += dA_ij[:, None] * k_j[None, :] * tl.math.exp2(bqg - gk_j[None, :])

    p_bdq = tl.make_block_ptr(
        dq + i_bh * s_qk_h,
        (T, DK),
        (s_qk_t, s_qk_d),
        (i_t * BT + i_c * BC, 0),
    ...
    for i in range(i_c + 1, tl.cdiv(BT, BC)):
        dA_ij = tl.load(
            ...
            + i_bh * s_A_h
            + i_t * s_A_c
            + (i_c * BC + tl.arange(0, BC)[:, None])
            + (i * BC + tl.arange(0, BC)[None, :]) * BT
        ).to(k.dtype.element_ty)
        ...
        dk_i += tl.dot(dA_ij, bq, allow_tf32=False) * tl.math.exp2(
            gq_normalizer[None, :] - bkg
        )
    ...
    for i in range(0, BC):
        ...
        dA_ij = tl.load(
            ...
            + i_bh * s_A_h
            + i_t * s_A_c
            + (i_c * BC + tl.arange(0, BC))
            + (i + i_c * BC) * BT
        )
        dA_ij = tl.where(o_i <= i, dA_ij, 0)
        q_j = tl.load(
            q + i_bh * s_qk_h + (i_t * BT + i_c * BC + i) * DK + tl.arange(0, DK)
        )
        gq_j = tl.load(
            g + i_bh * s_qk_h + (i_t * BT + i_c * BC + i) * DK + tl.arange(0, DK)
        )
        dk_i += dA_ij[:, None] * q_j[None, :] * tl.math.exp2(gq_j[None, :] - bkg)

    p_bdk = tl.make_block_ptr(
        dk + i_bh * s_qk_h,
        (T, DK),
        (s_qk_t, s_qk_d),
        (i_t * BT + i_c * BC, 0),
...
class ChunkGLAFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, scale, initial_state, output_final_state):
        ...
        o = rearrange(q_g, "b h (n c) d -> b h n c d", c=BT) @ rearrange(
            h, "b h (n c) d -> b h n c d", c=DK
        )
    ...
    def backward(ctx, do, d_final_state=None):
        ...
        dq = (
            rearrange_back(
                rearrange_chunk(do, BT)
                @ rearrange_chunk(h, d_head_qk).transpose(-1, -2)
            )
            * scale
        )
