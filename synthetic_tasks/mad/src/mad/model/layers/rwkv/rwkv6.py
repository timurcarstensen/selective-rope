...
def time_mixer_rwkv6_wrapped_bf16(
    ...
    **kwargs,
    ...
    wkv6_cuda = load(
        ...
        sources=[
            os.path.join(
                base_path, "mad", "model", "layers", "rwkv", "cuda", "wkv6_op.cpp"
            ),
            os.path.join(
                base_path, "mad", "model", "layers", "rwkv", "cuda", "wkv6_cuda.cu"
            ),
        ],
        verbose=True,
        extra_cuda_cflags=[
    ...
    class TimeMixer_RWKV6(MyModule):
        def __init__(
            ...
            **kwargs,
            ...
            with torch.no_grad():
                ratio_0_to_1 = (
                    0 if n_layer < 2 else layer_id / (n_layer - 1)
                )  # layer_id / (n_layer - 1)  # 0 to 1
                ...
                self.time_maa_w1 = nn.Parameter(
                    torch.zeros(dim, TIME_MIX_EXTRA_DIM * 5).uniform_(-1e-4, 1e-4)
                )
                self.time_maa_w2 = nn.Parameter(
                    torch.zeros(5, TIME_MIX_EXTRA_DIM, dim).uniform_(-1e-4, 1e-4)
                )

                # fancy time_decay
                decay_speed = torch.ones(dim_att)
                for n in range(dim_att):
                    decay_speed[n] = -6 + 5 * (n / (dim_att - 1)) ** (
                ...
                self.time_decay_w1 = nn.Parameter(
                    torch.zeros(dim, TIME_DECAY_EXTRA_DIM).uniform_(-1e-4, 1e-4)
                )
                self.time_decay_w2 = nn.Parameter(
                    torch.zeros(TIME_DECAY_EXTRA_DIM, dim_att).uniform_(-1e-4, 1e-4)
                )
            ...
            self.ln_x = nn.GroupNorm(
                self.n_head, dim_att, eps=(1e-5) * (head_dim_divisor**2)
            )
