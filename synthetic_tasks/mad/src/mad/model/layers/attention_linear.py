...
try:
    from mad.model.layers.ops.causal_dot_prod import (
        causal_dot_product,
    )  # linear attention cuda kernel
except ImportError:
    print(
        "causal_dot_product not installed, using quadratic linear attention implementation!... "
    )
...
class LinearAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.assign_feature_map(
            feature_map=feature_map, tie_feature_map_qk=tie_feature_map_qk
        )
    ...
    def assign_feature_map(self, feature_map: str, tie_feature_map_qk: bool = False):
        if feature_map == "hedgehog":
            ...
            if tie_feature_map_qk:
                self.feature_map_q = self.feature_map_k = TaylorFeatureMap(
                    head_dim=self.head_qk_dim
                )
            else:
    ...
    def parallel_forward(
        ...
        if self.parallel_implementation == "quadratic" or causal_dot_product is None:
            ...
            self.parallel_implementation == "linear" and causal_dot_product is not None
        ):
            ...
            raise ValueError(
                f"Parallel implementation {self.parallel_implementation} not supported"
            )
