...
class RTF(nn.Module):
    def __init__(
        ...
        assert trunc_len > state_size, (
            f"Truncation length {trunc_len} must be larger than the state size {state_size}."
        )
        ...
        self.ab = nn.Parameter(
            init_fn((1 + bidirectional) * (self.D + self.num_a), self.N)
        )  # a, b parameters
        self.h_0 = nn.Parameter(
            torch.randn((1 + bidirectional) * self.D)
        )  # h_0 parameter
    ...
    def get_k(self, L, *args, **kwargs):
        ...
        assert L <= self.L
        ab = pad(
            self.ab, (1, self.L - self.N - 1 + self.L % 2)
        )  # zero padding params. +self.L%2 is rFFT specific
        ab[: self.a_channels, 0] = 1.0  # setting the monic term
        AB = rfft(ab, dim=-1)  # polynomial evaluation on points of unity
        K = (
            AB[self.a_channels :]
            / repeat(AB[: self.a_channels], "D N -> (D R) N", R=self.D // self.num_a)
            + self.h_0[:, None]
        )  # get kernel spectrum
        k = irfft(K, dim=-1)[:, :L]  # return time domain kernel
        if self.bdir:
            k = torch.cat(
                (k[: self.D], k[self.D :].flip(-1)), dim=-1
            )  # flip half of the kernels
        ...
        if self.flash_fft_conv is not None:
            if self.bdir:
                raise NotImplementedError(
                    "Strange behavior with FlashFFTConv, not allowing non-causal convolutions."
                )
            ...
            if self.bdir:
                ...
                Y = KU[: self.D].T * rearrange(
                    KU[self.D :], "(B D) L -> B L D", D=self.D
                )
    ...
    def step(self, u, x_i):
        assert not self.bdir
        c = self.get_c()  # c can be cached
        a = repeat(
            self.ab[: self.a_channels], "D N -> (D R) N", R=self.D // self.num_a
        )  # repeated a can be cached
    ...
    def get_c(self, *args, **kwargs):
        ...
        I_AL = repeat(
            torch.eye(N, device=device) - torch.matrix_power(A, self.L),
            "C N M -> (C R) N M",
            R=self.D // self.num_a,
        )  # (I-A^L)
        return torch.linalg.solve(
            I_AL, self.ab[self.a_channels :], left=True
        )  # solves for C in, C_prime = C(I-A^L)
...
def xavier_init(
    channels, order
):  # xavier init can sometimes initialize an unstable system
    stdv = 1.0 / math.sqrt(order)
