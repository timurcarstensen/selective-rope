...
class T2RFeatureMap(nn.Module):
    def __init__(self, head_dim: int, dot_dim: int = None) -> T2RFeatureMap:
        super().__init__()
        # Trainable map
        if dot_dim is None:
            dot_dim = head_dim
...
class DPFPFeatureMap(nn.Module):
    def __init__(self, head_dim: int, nu: int = 4) -> DPFPFeatureMap:
        super().__init__()
        self.nu = nu
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_rolled = torch.cat(
            [x.roll(shifts=j, dims=-1) for j in range(1, self.nu + 1)], dim=-1
        )
...
class RebasedFeatureMap(nn.Module):
    ...
    def forward(self, x: torch.Tensor, flatten: bool | None = True):
        if self.use_beta and self.use_gamma and self.normalize:
            ...
            raise RuntimeError(
                f"Not supported combination of `use_gamma`, `use_beta` and `normalize`, "
        ...
        return torch.cat(
            [x2_2 * self.head_dim**-0.5, x2_1 * (2 / self.head_dim) ** 0.5], dim=-1
        )
