...
def get_result_paths(
    model_id: str, logs_path: str, task: str = "all", check_present: bool = True
) -> list:
    """
...
def aggregate_model_results(
    ...
    result_paths = get_result_paths(
        logs_path=logs_path, model_id=model_id, task=task, check_present=True
    )
    ...
    if size_before != size_after:
        print(
            f"/!\ dropped {size_before - size_after} rows from results because they contained NaNs"
        )

    if subset_to_best_sweep_runs:
        results_df = subset_model_results_to_best_sweep_runs(
            results=results_df, metric=metric, metric_criterion=metric_criterion
        )
...
def subset_model_results_to_best_sweep_runs(
    results: pd.DataFrame, metric: str = "test_acc", metric_criterion: str = "max"
) -> pd.DataFrame:
    """
    ...
    metric_columns = [
        ...
        if c.startswith("train_") or c.startswith("test_") and c != metric
    ]
    grouping_columns = [
        c
        for c in grouping_columns
        if c not in {"lr", "weight_decay"} and c not in metric_columns and c != "index"
    ]
    ...
    def find_best_performance(group):
        if metric_criterion == "max":
            ...
            raise ValueError(
                f'invalid metric_criterion "{metric_criterion}", must be one of "min", "max"'
            )
        return best_row

    best_results_from_sweep = results_grouped.apply(find_best_performance)
    best_results_from_sweep = best_results_from_sweep.reset_index(drop=True).drop(
        "index", axis=1
    )
