...
def generate_vocab_permutations(
    vocab, token_motif_size: int = 1, rng=None, *args, **kwargs
):
    """
...
def generate_in_context_recall_instance(
    ...
    **kwargs,
    ...
    if frac_noise > 0:
        ...
        noise_vocab = np.arange(
            non_special_vocab_size, non_special_vocab_size + noise_vocab_size
        )
    ...
    not_noise_idx = rng.choice(
        num_kv_pairs
    )  # make sure we have at least one key-value pair in the sequence
    for i in range(
        num_kv_pairs - 1
    ):  # subtract one to account for final key-value pair
        # determine if noise or kv pair collected:
        is_noise = (
            rng.random() < frac_noise
    ...
    **kwargs,
    ...
    if is_training:
        ...
        for motif_size in range(1, k_motif_size + 1):
            keys_size = generate_vocab_permutations(
                key_vocab, token_motif_size=motif_size, rng=rng
            )
            keys[motif_size] = keys_size
        values = {}
        for motif_size in range(1, v_motif_size + 1):
            values_size = generate_vocab_permutations(
                value_vocab, token_motif_size=motif_size, rng=rng
            )
            values[motif_size] = values_size
    else:
        # we always prompt at the maximum key motif size
        keys = {
            k_motif_size: generate_vocab_permutations(
                key_vocab, token_motif_size=k_motif_size, rng=rng
            )
        }
        values = {}
        for motif_size in range(1, v_motif_size + 1):
    ...
    probe_added = (
        False  # flag indicating whether key-value probe has been added to the sequence
    )
...
def generate_kv_map(
    ...
    **kwargs,
    ...
    """
    kv_map_rng = np.random.default_rng(
        seed
    )  # fixed rng to ensure stable kv mapping across instances
...
def generate_memorization_instance(
    ...
    **kwargs,
    ...
    for i in range(
        num_kv_pairs
    ):  # subtract one to account for the final key-value pair
        # determine if noise or key-value pair collected:
        is_noise = (
            rng.random() < frac_noise
            if i != not_noise_idx and frac_noise > 0
...
def generate_copying_instance(
    ...
    **kwargs,
    ...
    # generate inputs:
    assert seq_len > (num_tokens_to_copy * 2) + 1, (
        "seq_len must be > (num_tokens_to_copy * 2) + 1"
    )
    ...
    if not selective:
        ...
        inputs = np.insert(
            inputs, insert_indices, [blank_token] * num_blank_tokens
        ).tolist()
    inputs += [copy_token]
