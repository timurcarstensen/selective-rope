...
def _generate_instance(args):
    """Top-level helper for multiprocessing (closures are not picklable)."""
    instance_fn, instance_fn_kwargs = args
    return instance_fn(**instance_fn_kwargs)


def _generate_and_write_instance(args):
    """Top-level helper for multiprocessing (DiskDataset variant)."""
    instance_fn, instance_fn_kwargs, doc_path = args
    instance = instance_fn(**instance_fn_kwargs)
    if len(instance) == 2:
        instance_dict = {
        ...
        raise ValueError(
            f"returned instances need to contain 2 entries but got {len(instance)}"
        )
    with open(doc_path, "a+") as fp:
        json.dump(instance_dict, fp)
        fp.write("\n")
...
def generate_data(
    ...
    if train_data_path is not None and train_data_exists:
        ...
        training_dataset.generate_data(
            num_examples=num_train_examples, num_workers=num_workers
        )
    ...
    if test_data_path is not None and test_data_exists:
        ...
        test_dataset.generate_data(
            num_examples=num_test_examples, num_workers=num_workers
        )
...
class MemoryDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, instance_fn: callable, instance_fn_kwargs: dict, verbose: bool = True
    ) -> None:
        super().__init__()
    ...
    def __getitem__(self, idx):
        assert self.inputs is not None, (
            "no data generated yet; call self.generate_data!"
        )
    ...
    def generate_data(
        ...
        if self.verbose:
            print(
                f"Generating dataset with {num_examples} examples, using {num_workers} workers..."
            )

        # parallel data generation:
        if num_workers > 1:
            args = [(self.instance_fn, self.instance_fn_kwargs)] * num_examples
            pool = mp.Pool(num_workers)
            instances = pool.map(_generate_instance, args)
            pool.close()

        # sequential data generation:
        else:
            iterator = (
                tqdm(range(num_examples)) if self.verbose else range(num_examples)
            )
...
def generate_data_disk(
    ...
    **kwargs,
    ...
    Returns:
        dict: dictionary with keys 'train' and 'test' containing torch datasets
    """
    assert not os.path.isdir(train_data_path), (
        f'directory "{train_data_path}" already exists'
    )
    assert not os.path.isdir(test_data_path), (
        f'directory "{test_data_path}" already exists'
    )
...
class DiskDataset(torch.utils.data.Dataset):
    ...
    def use_data_from_idx(self, data_idx: str | pd.DataFrame):
        """
        ...
        """
        self.data_idx = (
            pd.read_csv(data_idx, index_col=0)
        ...
        assert len(self.data_idx) == sum(
            [len(pd.read_csv(d, index_col=0)) for d in self.documents]
        )

    def generate_data(
        self, path: str, num_examples: int, num_documents: int = 1, num_workers: int = 0
    ):
        """
    ...
    def generate_doc_data(
        ...
        if num_workers > 1:
            args = [
                (self.instance_fn, self.instance_fn_kwargs, doc_path)
            ] * num_examples
            pool = mp.Pool(num_workers)
            pool.map(_generate_and_write_instance, args)
            pool.close()
    ...
    def write_instance_to_doc(self, instance: tuple, doc_path: str):
        """
        ...
        if len(instance) == 2:
            ...
            raise ValueError(
                f"returned instances need to contain 2 (or 3) entries but got {len(instance)}"
            )
