...
def plot(
    ...
    plot_df["model"] = (
        plot_df["model.sequence_mixer.name"].map(model_tag_2_name)
        + " (K="
        + plot_df["model.sequence_mixer.kwargs.feature_dim"].astype(str)
        + ")"
        + " (H="
        + plot_df["model.sequence_mixer.kwargs.num_key_value_heads"].astype(str)
        + ")"
    )
