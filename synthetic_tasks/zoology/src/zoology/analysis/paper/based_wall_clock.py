...
def profile_mamba2(
    batch, seq_len, num_heads, head_dim, dstate, use_tk, chunk_size=64, ngroups=1
):
    dtype = torch.bfloat16
    device = torch.device("cuda")

    batch, seq_len, num_heads, head_dim, dstate = (
        int(batch),
        int(seq_len),
        int(num_heads),
        int(head_dim),
...
def profile_mamba1(
    batch, seq_len, num_heads, head_dim, dstate, chunk_size=64, ngroups=1
):
...
def profile_attn(batch, seq_len, num_heads, head_dim, block_size, use_tk):
    ...
    for _ in range(10):
        flash_attn_func(
            ...
            softmax_scale=0.5,
            causal=True,
            window_size=(int(block_size), int(block_size)),
        )
...
def profile_based(batch, seq_len, num_heads, head_dim, feature_dim, use_tk):
    ...
    for _ in range(10):
        if use_tk:
            ...
            fused_chunk_based(q, k, v, True, True)
    torch.cuda.synchronize()
    t1 = time.time()
    elapsed = (t1 - t0) / 10
    return elapsed
...
def plot(
    ...
    idx = df.groupby(["state_size", "model.name"])[metric].idxmax(skipna=True).dropna()
    plot_df = df.loc[idx]
    ...
    for i, row in plot_df.iterrows():
        ...
        if model_name in model_name_2_func:
            ...
            if model_name == "based":
                feature_dim = row[
                    "model.sequence_mixer.kwargs.configs.1.kwargs.feature_dim"
                ]
                ...
                block_size = row[
                    "model.sequence_mixer.kwargs.configs.1.kwargs.block_size"
                ]
    ...
    g.set(
        xscale="log",
        ylabel="Associative Recall Accuracy",
