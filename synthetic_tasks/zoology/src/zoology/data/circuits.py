...
def parity(
    vocab_size: int, num_examples: int, input_seq_len: int, seed: int, **kwargs
) -> DataSegment:
    """
    ...
    for _ in range(num_examples):
        ...
        label = torch.full_like(
            input[:-1], -100
        )  # -100 for labels, except last position
        label[-1] = input[-1]
        input = input[:-1]
    ...
    return DataSegment(inputs, labels, slices={"input_seq_len": input_seq_len})
...
def vocab_majority(
    ...
    return DataSegment(inputs, labels, slices={"vocab_size": vocab_size})
...
def cumulative_majority(
    config: CumulativeMajorityConfig, seed: int, **kwargs
) -> DataSegment:
    """
    Generate majority sequences.
    """
