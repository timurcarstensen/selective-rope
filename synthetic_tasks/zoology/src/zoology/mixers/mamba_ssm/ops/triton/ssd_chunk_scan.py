...
def _chunk_scan_fwd_kernel_wip(
    ...
    for start_m in range(0, chunk_size_limit, BLOCK_SIZE_M):
        ...
        if start_m + BLOCK_SIZE_M < chunk_size_limit:
            ...
            # TODO: seq_idx
            tl.exp(dA_cs_last - dA_cs_m) * dt_m
            # B *= scale
...
def _chunk_scan_fwd_wip(
    ...
    def grid(META):
        return (triton.cdiv(headdim, META["BLOCK_SIZE_N"]), batch * nchunks, nheads)
...
def _chunk_scan_bwd_dz(
    ...
    def grid_dz(META):
        return (triton.cdiv(chunk_size, META["BLOCK_SIZE_M"]), batch * nchunks, nheads)
