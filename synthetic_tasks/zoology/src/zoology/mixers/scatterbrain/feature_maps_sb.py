...
from zoology.mixers.scatterbrain.projection_utils import (
...
def softmax_kernel(
    ...
    *,
    ...
    data_dash = torch.einsum(
        "...id,jd->...ij", data, data_normalizer * projection_matrix
    )
    ...
    if is_query:
        log_scale = (
            -diag_data + torch.amax(data_dash, dim=-1, keepdim=True) - math.log(m) / 2
        )
...
class SBPerformerFeatures(FeatureMap):
    ...
    def __init__(
        ...
        self.softmax_temp = (
            1 / math.sqrt(query_dims) if softmax_temp is None else softmax_temp
        )
        self.cosh = cosh
        # self.redraw = redraw
        ...
        self.register_buffer(
            "projection_matrix",
            torch.zeros(self.query_dims, self.n_features, **factory_kwargs),
        )
