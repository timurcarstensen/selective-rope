...
def causal_linear_attention(q, k, v, need_weights=False):
    ...
    assert not is_half or APEX_AVAILABLE, (
    ...
    causal_dot_product_fn = (
        amp.float_function(causal_dot_product) if is_half else causal_dot_product
    )
    with cuda_context():
        if autocast_enabled:
            q, k, v = map(lambda t: t.float(), (q, k, v))

        q_k_v = causal_dot_product_fn(
            q, k, v
        )  # q.unsqueeze(1), k.unsqueeze(1), v.unsqueeze(1)).squeeze(1)
        if need_weights:
