...
class GlobalPerformer(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        global_out_cumsum = torch.einsum(
            "...nm,...nm->...n", q_prime, k_prime.cumsum(dim=-2)
        )
        ...
        scores = (
            torch.einsum("b h m d, b h n d -> b h m n", q_prime, k_prime)
            * self.softmax_scale
        )
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        # scatterbrain
        self.get_global = GlobalPerformer(
            dim_heads=dim_heads,
    ...
    def forward(self, qkv, key_padding_mask=None):
        ...
        """
        _batch_size, _seqlen = qkv.shape[0], qkv.shape[1]
        ...
        assert Hv == Hk, (
            "The number of heads in the key and value tensor must be the same."
        )
        assert S == Tk, (
            "The sequence length of the key and value tensor must be the same."
        )
        1 / math.sqrt(E)
        ...
        # Global attn
        global_out, global_out_cumsum, global_scale, global_qk = self.get_global(
            query, key, value
        )

        # Local attn (SWA)
        scores = (
            torch.einsum("b h m d, b h n d -> b h m n", query, key) * self.softmax_scale
        )
        ...
        global_log_normalization = (
            torch.log(
                (
                    rearrange(global_out_cumsum, "b h s -> b h s 1") - global_qk_sum
                ).clamp_min_(1e-24)
            )
            + global_scale
        )
        ...
        local_score = (
            self.dropout(torch.softmax(local_qk - log_normalization, dim=-1))
            - global_qk * global_prime_scale
        )
...
class SBLocalAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        2 * self.head_dim * self.num_heads_kv
