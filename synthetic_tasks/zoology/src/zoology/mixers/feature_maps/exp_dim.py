...
class FullSpaceMap(nn.Module):
    ...
    def __init__(self, head_dim_idx: int = -1, eps: float = 1e-12, **kwargs: any):
        super().__init__()
...
class ExpDim(FeatureMap):
    ...
    def __init__(self, fullspace: bool = True, **kwargs):
...
class ExpSumNorm(ExpDim):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.stack([x >= 0, x < 0], dim=self.head_dim_idx) * x.abs().softmax(
            dim=self.head_dim_idx
        ).unsqueeze(-1)
...
class ExpSumNormHalfspace(ExpDim):
    ...
    def forward(self, x: torch.Tensor):
        # x = x * self.temp  # b h l d 2
        return ((x >= 0) * x.abs().softmax(dim=self.head_dim_idx)).clamp(min=self.eps)
