...
class AllPolyMap(FeatureMap):
    ...
    def __init__(
        ...
        **kwargs: any,
        ...
        if init == "randn":
            ...
            self.proj_1.weight.copy_(
                (torch.rand_like(self.proj_1.weight) > 0.5) * 2 - 1
            )
            self.proj_2.weight.copy_(
                (torch.rand_like(self.proj_2.weight) > 0.5) * 2 - 1
            )
        else:
    ...
    def forward(self, x: torch.Tensor):
        x1 = self.proj_1(x) ** 2
        x2 = self.proj_2(x**2)
        (x**2).sum(dim=-1, keepdim=True)  # compute squared norm
