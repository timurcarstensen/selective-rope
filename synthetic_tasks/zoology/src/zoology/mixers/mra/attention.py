...
import torch
from autograd_function import ReduceSum, SampledDenseMM, SparseDenseMM
from kernel import sparse_mask_B, sparse_max
...
def get_low_resolution_logit(Q, K, block_size, mask=None, V=None):
    ...
    V_hat = None
    if mask is not None:
        token_count = mask.reshape(batch_size, num_block_per_row, block_size).sum(
            dim=-1
        )
        Q_hat = Q.reshape(batch_size, num_block_per_row, block_size, head_dim).sum(
            dim=-2
        ) / (token_count[:, :, None] + 1e-6)
        K_hat = K.reshape(batch_size, num_block_per_row, block_size, head_dim).sum(
            dim=-2
        ) / (token_count[:, :, None] + 1e-6)
        if V is not None:
            V_hat = V.reshape(batch_size, num_block_per_row, block_size, head_dim).sum(
                dim=-2
            ) / (token_count[:, :, None] + 1e-6)
    else:
        token_count = block_size * torch.ones(
            batch_size, num_block_per_row, dtype=torch.float, device=Q.device
        )
        Q_hat = Q.reshape(batch_size, num_block_per_row, block_size, head_dim).mean(
            dim=-2
        )
        K_hat = K.reshape(batch_size, num_block_per_row, block_size, head_dim).mean(
            dim=-2
        )
        if V is not None:
            V_hat = V.reshape(batch_size, num_block_per_row, block_size, head_dim).mean(
                dim=-2
            )

    low_resolution_logit = torch.matmul(Q_hat, K_hat.transpose(-1, -2)) / math.sqrt(
        head_dim
    )

    low_resolution_logit_row_max = low_resolution_logit.max(
        dim=-1, keepdims=True
    ).values
    ...
    if mask is not None:
        low_resolution_logit = (
            low_resolution_logit
            - 1e4 * ((token_count[:, None, :] * token_count[:, :, None]) < 0.5).float()
        )
...
def get_block_idxes(
    ...
    if initial_prior_diagonal_n_blocks > 0:
        ...
        temp_mask = torch.ones(
            ...
            total_blocks_per_row,
            device=low_resolution_logit.device,
        )
        diagonal_mask = torch.tril(
            torch.triu(temp_mask, diagonal=-offset), diagonal=offset
        )
    ...
    top_k_vals = torch.topk(
        low_resolution_logit.reshape(batch_size, -1),
        num_blocks,
        dim=-1,
        largest=True,
        sorted=False,
    ...
    if approx_mode == "full":
        ...
        high_resolution_mask = (
            low_resolution_logit >= threshold[:, None, None]
        ).float()
...
def mra2_attention(
    ...
    if approx_mode == "full":
        low_resolution_logit, token_count, low_resolution_logit_row_max, V_hat = (
            get_low_resolution_logit(Q, K, block_size, mask, V)
        )
    elif approx_mode == "sparse":
        with torch.no_grad():
            low_resolution_logit, token_count, low_resolution_logit_row_max, _ = (
                get_low_resolution_logit(Q, K, block_size, mask)
            )
    else:
        raise Exception()
    ...
    with torch.no_grad():
        low_resolution_logit_normalized = (
            low_resolution_logit - low_resolution_logit_row_max
        )
    ...
    high_resolution_logit = SampledDenseMM.operator_call(
        Q, K, indices, block_size=block_size
    ) / math.sqrt(head_dim)
    max_vals, max_vals_scatter = sparse_max(
        high_resolution_logit, indices, num_block_per_row, num_block_per_row
    )
    high_resolution_logit = high_resolution_logit - max_vals_scatter
    if mask is not None:
        high_resolution_logit = high_resolution_logit - 1e4 * (
            1 - sparse_mask_B(mask, indices)[:, :, :, None]
        )
    high_resolution_attn = torch.exp(high_resolution_logit)
    high_resolution_attn_out = SparseDenseMM.operator_call(
        high_resolution_attn, indices, V, num_block_per_row
    )
    ...
    if approx_mode == "full":
        low_resolution_attn = (
            torch.exp(
                low_resolution_logit
                - low_resolution_logit_row_max
                - 1e4 * high_resolution_mask
            )
            * token_count[:, None, :]
        )
        ...
        low_resolution_normalizer = (
            low_resolution_attn.sum(dim=-1)[:, :, None]
            .repeat(1, 1, block_size)
            .reshape(meta_batch, seq_len)
        )

        log_correction = (
            low_resolution_logit_row_max.repeat(1, 1, block_size).reshape(
                meta_batch, seq_len
            )
            - max_vals
        )
        ...
        low_resolution_attn_out = (
            low_resolution_attn_out * low_resolution_corr[:, :, None]
        )
        ...
        high_resolution_attn_out = (
            high_resolution_attn_out * high_resolution_corr[:, :, None]
        )
        ...
        attn = (high_resolution_attn_out + low_resolution_attn_out) / (
            high_resolution_normalizer[:, :, None]
            + low_resolution_normalizer[:, :, None]
            + 1e-6
        )

    elif approx_mode == "sparse":
        attn = high_resolution_attn_out / (
            high_resolution_normalizer[:, :, None] + 1e-6
        )
    else:
        raise Exception()
