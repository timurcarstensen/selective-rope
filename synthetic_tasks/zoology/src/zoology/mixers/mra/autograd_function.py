import torch
from kernel import mm_to_sparse, reduce_sum, sparse_dense_mm, transpose_indices
...
class SampledDenseMM(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        grad_B = sparse_dense_mm(
            grad.transpose(-1, -2), indices_T, dense_A, B_num_block
        )
...
class SparseDenseMM(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        indices_T = transpose_indices(indices, A_num_block, B_num_block)
        grad_B = sparse_dense_mm(
            sparse_A.transpose(-1, -2), indices_T, grad, B_num_block
        )
