...
def sparse_max_cuda(sparse_C, indices, A_num_block, B_num_block):
    ...
    max_vals, max_vals_scatter = cuda_kernel.index_max(
        index_vals, indices, A_num_block, B_num_block
    )
...
def sparse_to_dense(sparse_A, indices, A_num_block, block_size=32):
    ...
    eyes = torch.eye(A_num_block * block_size, device=sparse_A.device).repeat(
        batch_size, 1, 1
    )
    eyes = eyes.reshape(
        batch_size, A_num_block, block_size, A_num_block * block_size
    ).transpose(-1, -2)
    dense_A = sparse_dense_mm_torch(sparse_A, indices, eyes, A_num_block)
    dense_A = dense_A.transpose(-1, -2).reshape(
        batch_size, A_num_block * block_size, A_num_block * block_size
    )
    return dense_A
...
def mm_to_sparse_torch(dense_A, dense_B, indices, block_size=32):
    ...
    dense_A = dense_A[
        batch_idx[:, None],
        torch.div(indices, B_num_block, rounding_mode="floor").long(),
        :,
        :,
    ]
...
def mm_to_sparse(dense_A, dense_B, indices, block_size=32, cuda=True):
    ...
    dense_A = dense_A.reshape(
        batch_size, A_size // block_size, block_size, dim
    ).transpose(-1, -2)
    dense_B = dense_B.reshape(
        batch_size, B_size // block_size, block_size, dim
    ).transpose(-1, -2)
...
def sparse_dense_mm_torch(sparse_A, indices, dense_B, A_num_block):
    ...
    global_idxes = (
        torch.div(indices, B_num_block, rounding_mode="floor").long()
        + batch_idx[:, None] * A_num_block
    ).reshape(batch_size * num_block)
    temp = torch.zeros(
        (batch_size * A_num_block, dim * block_size),
        dtype=dense_C.dtype,
        device=dense_C.device,
    )
    dense_C = temp.index_add(0, global_idxes, dense_C).reshape(
        batch_size, A_num_block, dim, block_size
    )
...
def sparse_dense_mm(sparse_A, indices, dense_B, A_num_block, block_size=32, cuda=True):
    ...
    dense_C = dense_C.transpose(-1, -2).reshape(
        batch_size, A_num_block * block_size, dim
    )
    return dense_C
...
def reduce_sum_torch(sparse_A, indices, A_num_block, B_num_block):
    ...
    temp = torch.zeros(
        (batch_size * A_num_block, block_size),
        dtype=sparse_A.dtype,
        device=sparse_A.device,
    )
    output = temp.index_add(0, global_idxes, sparse_A).reshape(
        batch_size, A_num_block, block_size
    )
...
def transpose_indices(indices, dim_1_block, dim_2_block):
    return (
        (indices % dim_2_block) * dim_1_block
        + torch.div(indices, dim_2_block, rounding_mode="floor")
    ).long()
