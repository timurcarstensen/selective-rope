...
class BigBIRDSelfAttention(nn.Module):
    ...
    def forward(self, qkv):
        _batch_size, l, d_model = qkv.shape[0], qkv.shape[1], qkv.shape[2]
        ...
        positions = (
            torch.randint(l - 1, (self.random_toks,)),
            torch.randint(l - 1, (self.random_toks,)),
        )
        positions = (
        ...
        # causal masking
        causal_mask = torch.triu(torch.full((l, l), -20000.0), diagonal=1).to(
            scores.device
        )
...
class BigBIRDAttention(nn.Module):
    def __init__(
        self, d_model, block_size=128, bias=True, dropout=0.0, **kwargs
    ) -> None:
        super().__init__()
        self.d_model = d_model
        self.block_size = block_size
        self.Wqkv = nn.Linear(d_model, 3 * d_model, bias=bias)
        self.inner_attn = BigBIRDSelfAttention(
            block_size=block_size, attention_dropout=dropout
        )
