...
from zoology.mixers.nystromformer.attn import Attention, AttentionConfig
from zoology.mixers.nystromformer.core import (
    scaled_dot_product_attention,
    scaled_query_key_softmax,
)
from zoology.mixers.nystromformer.utils import (
...
class NystromSelfAttentionConfig(AttentionConfig):
    ...
    landmark_pooling: nn.Module | None
    causal: bool | None
    pinverse_original_init: bool | None
    inv_iterations: int | None
    v_skip_connection: nn.Module | None
    conv_kernel_size: int | None
    use_razavi_pinverse: bool | None
...
class NystromAttention(Attention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.causal_mask_1: torch.Tensor | None = None
        self.causal_mask_2: torch.Tensor | None = None
        self.causal_mask_3: torch.Tensor | None = None
        ...
        qkv_dim = self.head_dim * (self.num_heads + 2 * self.num_heads)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.num_landmarks >= seq_len:
            ...
            mask_3: torch.Tensor | None = self.causal_mask_3
            if key_padding_mask is not None:
                mask_3 = (
            ...
            kernel_1 = scaled_query_key_softmax(
                q=q, k=k_landmarks, att_mask=self.causal_mask_1
            )
            kernel_2 = scaled_query_key_softmax(
                q=q_landmarks, k=k_landmarks, att_mask=self.causal_mask_2
            )
    ...
    def state_size(self, sequence_length: int = 2048):
        return 2 * self.d_model * self.num_landmarks
