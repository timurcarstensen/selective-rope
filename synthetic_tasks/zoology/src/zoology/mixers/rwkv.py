...
):  # fancy initialization of all lin & emb layer in the model
    ...
    for mm in model.modules():
        ...
        with torch.no_grad():
            ...
            for (
                name,
                parameter,
            ) in model.named_parameters():  # find the name of the weight
...
class RWKVTimeMixer(torch.jit.ScriptModule):
    def __init__(
        self, l_max: int, d_model: int = 512, n_layer: int = 12, layer_idx: int = -1
    ):
        super().__init__()
...
class RWKVStateMixer(torch.jit.ScriptModule):
    def __init__(self, d_model=512, n_layer=12, layer_idx=-1):
        super().__init__()
        self.layer_id = layer_idx
