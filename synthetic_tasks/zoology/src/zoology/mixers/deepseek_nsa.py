...
from torch import arange, cat, nn, stack, tensor
...
try:
    ...
    print("Please install rotary-embedding-torch: pip install rotary-embedding-torch")

# einstein notation
try:
    import einx
except:
    print("Please install einops: pip install einx")
...
class SparseAttention(Module):
    def __init__(
        ...
        if num_selected_blocks == 0:
            print(
                "`num_selected_blocks` should be set greater than 0, unless if you are ablating it for experimental purposes"
            )
    ...
    def forward(
        ...
        batch, seq_len, _scale, _heads, device = (
            *inp.shape[:2],
            self.scale,
            self.heads,
            inp.device,
        ...
        assert seq_len % compress_divisible_seq_len == 0, (
            "sequence length must be divisible by compress block size"
        )
        ...
        if self.causal:
            ...
            ck_seq = (
                (arange(num_compress_blocks, device=device) + 1)
        ...
        if has_selected_kv_for_fine_attn:
            ...
            if self.use_triton_kernel and not disable_triton_kernel:
                ...
                fmask = selected_importance_values > 1e-10
                assert seq_len == fine_divisible_seq_len, (
                    "sequence length must be divisible by selection block size"
                )

                if self.causal:
                    # handle block causal diagonal in the diagram, but run experiments without to see
                    fine_window_seq = (
                        arange(fine_divisible_seq_len, device=device)
                        // self.selection_block_size
                    )
                    ...
                    fmask = repeat(
                        fmask, "b h i w -> b h 1 i (w j)", j=self.selection_block_size
                    )
        ...
        out = einsum(
            ...
            stack([compressed_attn_out, fine_attn_out, sliding_window_attn_out]),
    ...
    def state_size(self, sequence_length: int = 2048):
        ...
        state_size += (
            2 * self.selection_block_size * self.dim * self.num_selected_blocks
        )
        return state_size
