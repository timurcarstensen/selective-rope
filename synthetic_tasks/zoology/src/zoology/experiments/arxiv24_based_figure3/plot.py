...
def plot(
    ...
    for x_col in ["num_parameters", "state_size"]:
        idx = df.groupby([x_col, "feature_map"])[metric].idxmax(skipna=True).dropna()
        plot_df = df.loc[idx]
...
if __name__ == "__main__":
    df = fetch_wandb_runs(
        launch_id=["default-2024-02-14-23-41-12"], project_name="zoology"
    )
    ...
    df = df[
        ...
        | (
            df["model.sequence_mixer.kwargs.configs.1.kwargs.feature_name"]
