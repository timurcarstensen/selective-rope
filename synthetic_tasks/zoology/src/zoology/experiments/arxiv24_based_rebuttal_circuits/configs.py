...
for k in [16, 32, 64, 128]:
    train_configs.append(
        CumulativeParityConfig(
            vocab_size=VOCAB_SIZE, input_seq_len=k, num_examples=100_000
        )
    )
    test_configs.append(
        CumulativeParityConfig(
            vocab_size=VOCAB_SIZE, input_seq_len=k, num_examples=1_000
        )
    )
...
# test_configs = [CumulativeParityConfig(vocab_size=VOCAB_SIZE, input_seq_len=SEQLEN, num_examples=1_000),]
...
# cumulative sum
for d_model in [128]:
    mixer = dict(name="zoology.mixers.cumsum.CumSum", kwargs={})
    model = ModelConfig(
        block_type="TransformerBlock",
        d_model=d_model,
        n_layers=1,
