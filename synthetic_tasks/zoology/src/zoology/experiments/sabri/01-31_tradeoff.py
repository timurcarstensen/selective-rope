...
data = DataConfig(
    ...
    builder={
        ...
        "kwargs": {"train_configs": train_configs, "test_configs": test_configs},
    },
)


def get_sequence_mixers(sequence_mixer: str, d_model: int):
    n_layers = 2
        ...
        for ftr_dim in [16]:
            for slide_width in [8, 32, 128]:
                ...
                slide_attn = dict(
                    ...
                    kwargs={"block_size": slide_width, "attention_dropout": 0.0},
                )
                mixer = dict(
                    name="zoology.mixers.hybrid.Hybrid",
                    kwargs={"configs": [conv_mixer, slide_attn, lin_attn]},
                )
                name = f"based-{ftr_dim=}-{slide_width=}"
                n_layers = 3
                sequence_mixers.append((name, mixer))
    if sequence_mixer == "slide":
        block_type = "TransformerBlock"
        for slide_width in [8, 16, 32, 64, 128, 256, 512, 1024]:
            ...
            n_layers = 2
            sequence_mixers.append((name, mixer))
    elif sequence_mixer == "based":
        block_type = "TransformerBlock"
        for ftr_dim in [8, 16, 24]:
            lin_attn = dict(
                name="zoology.mixers.based.Based",
                kwargs={
                    "l_max": input_seq_len,
...
for lr in np.logspace(-3, -1.5, 4):
    for d_model in [
        ...
        for sequence_mixer in [
            ...
            sequence_mixers, block_type, n_layers = get_sequence_mixers(
                sequence_mixer, d_model
            )
            ...
            for name, mixer in sequence_mixers:
                ...
                config = TrainConfig(
                    ...
                    learning_rate=lr,
                    max_epochs=64,
                    logger=LoggerConfig(project_name="zoology", entity="hazy-research"),
                    sweep_id=sweep_name,
                    run_id=run_id,
