...
test_configs = [
    ...
    MQARConfig(
        vocab_size=VOCAB_SIZE, input_seq_len=2048, num_examples=1_000, num_kv_pairs=512
    ),
]
...
for d_model in [64, 128, 256]:
    for d_state in [8, 16, 24]:
        mixer = dict(name="zoology.mixers.mamba.Mamba", kwargs={"d_state": d_state})
        model = ModelConfig(
            block_type="MambaBlock",
            d_model=d_model,
            n_layers=2,
...
block_type = "TransformerBlock"
for d_model in [64, 128, 256]:
    mixer = dict(name="zoology.mixers.hyena.Hyena", kwargs={"l_max": input_seq_len})
    model = ModelConfig(
        block_type="TransformerBlock",
        d_model=d_model,
        n_layers=2,
...
for d_model in [64, 128, 256]:
    mixer = dict(
        ...
        kwargs={"l_max": input_seq_len, "d_state": d_model / 4, "head_dim": 2},
    )
    model = ModelConfig(
        block_type="TransformerBlock",
        d_model=d_model,
