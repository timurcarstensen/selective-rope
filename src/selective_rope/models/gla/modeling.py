...
from selective_rope.models.utils import Cache
...
from selective_rope.layers.attn import Attention
from selective_rope.layers.gla import GatedLinearAttention
    ...
    def __init__(self, config: GLAConfig, layer_idx: int):
        ...
        if config.attn is not None and layer_idx in config.attn["layers"]:
            self.attn = Attention(
                ...
                layer_idx=layer_idx,
                position_embedding={"type": "rope", "theta": config.attn["rope_theta"]},
            )
        else:
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn(
                "`GLAModel` does not `output_attentions` now, setting it to `False`."
            )
